...
class encoderTest():
    def __init__(self, path, file, report):
        ...
        self.encoder = Encoder.Encoder(self.nodes, self.nodes[:,3], file, depotBits=1)
...
if __name__ == '__main__':
    ...
    report.print = reportFormat
    report.fails = [[set(), set(), set(), set()] for i in range(6)]
    
    # iterate through all the files
    for dir in os.listdir(os.path.dirname(__file__) + '/../../data/test/'):
        path = os.path.dirname(__file__) + '/../../data/test/' + dir
        for file in os.listdir(path): report, test = main(path, file, report)
